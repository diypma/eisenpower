...
export default function TaskDetailModal({ isOpen, onClose, task, onToggleSubtask, onDelete }) {
    ...
    return (
        ...
        >
            <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-lg p-10 animate-in zoom-in-95 duration-200">
                <div className="flex justify-between items-start mb-6">
                    <div className="flex-1">
                        <h2 className="text-3xl font-black text-slate-800 leading-tight pr-4">{task.text}</h2>
                ...
                <div className="mb-8">
                    ...
                    <div className="space-y-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                        {task.subtasks?.map((sub) => (
                            <div
                                key={sub.id}
                                className="flex items-center gap-3 group/sub cursor-pointer p-2 hover:bg-slate-50 rounded-xl transition-all"
                ...
                `}>
                                ...
                                <span className={`font-semibold text-sm transition-all ${sub.completed ? 'text-slate-400 line-through' : 'text-slate-700'}`}>
                                    {sub.text}
                                </span>
                            </div>
                        ))}
                        {(!task.subtasks || task.subtasks.length === 0) && (
                            <div className="text-center py-6 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-100">
                                <p className="text-sm font-bold text-slate-300">No sub-tasks defined</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="flex gap-4 border-t border-slate-100 pt-8">
                    <button
                        ...
                        className="px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest text-red-500 hover:bg-red-50 transition-colors"
                    >
                        Delete Task
                    </button>
                    <div className="flex-1" />
